...
void makeItRain(int allCitiesCount) {
	...
	while (notAllCitiesVisited(allCitiesCount)) {
		...
		for (cityTo = 1; cityTo <= allCitiesCount; cityTo++) 
		{
			...
			if (currentTimes[cityTo] != 999999999)
				continue;
		...
		// pick best option -> move to next city
		cityFrom = makeMove();
	}
}
...
int main() {
	...
	for (mapId = 0; mapId < mapsCount; mapId++) {
		...
		for (int cityTime = 0; cityTime < citiesCount + 1; cityTime++) {				// plus one, so you can ignore 0 row and column	=> city 1 has index 1
